        ...
        <el-dialog :title="text_Text" :visible.sync="fileToggle" :before-close="userClose" width="68%">
            <div class="fileBox">
                ...
                <div class="fileBoxRight">
                    <el-upload class="upload-demo" :disabled="!upShowhide" :action="uploadUrl" :multiple="false" :on-success="handleSuccess" :file-list="fileList" list-type="picture-card" :on-preview="handlePictureCardPreview" :on-remove="delFile">
...
export default {
    ...
    methods: {
        btn_save() {
            ...
            this.newFileObj.newSrcstr = "";
            this.newFileObj.zdName = this.lsZdsave;
            if (this.fileList.length) {
                this.fileList.map(i => {
                    this.newFileObj.newSrcstr += i.src + ";";
                });
            } else {
                this.newFileObj.newSrcstr = "";
            }
            if (this.newFileObj.newSrcstr) {
                this.newFileObj.newSrcstr = this.newFileObj.newSrcstr.substring(
                    0,
                    this.newFileObj.newSrcstr.length - 1
                );
            }
            this.$emit("chileFile", this.newFileObj);
        ...
        handleSuccess(res) {
            if (res.success) {
                ...
                this.fileList.push({
                    ...
                    src: res.data,
                    url:
                        this.baseUrl + "/file/downloadFile?filePath=" + res.data
                });
            } else {
                this.$message({
                    type: "warning",
